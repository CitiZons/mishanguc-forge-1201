 ...
 * <p><b>Note: </b>This class is <i>minecraft-side only</i> since 0.1.7, as it uses minecraft-side {@link NbtClickEvent}. It means that the server does not pretty-print any NBT.</p>
 * <p>You can let the server send the NBT through a packet, and let the minecraft receive the packet and present NBT.</p>
 */
@OnlyIn(Dist.CLIENT)
public final class NbtPrettyPrinter {
  ...
  /**
   * These methods prettify an {@link Tag} to a {@link Text} in a way that is better than
   * {@link NbtUtils#toPrettyPrintedText(Tag)}. It dispatches to other methods by detecting and casting
   ...
   * @see NbtUtils#toPrettyPrintedText(Tag)
   */
  public static Component serialize(Tag element, int layer, String indent, int depth) {
    if (element instanceof final CompoundTag nbtCompound) {
      ...
      return TextBridge.literal(String.valueOf(nbtByte.getAsByte())).append(TextBridge.literal("b").withStyle(ChatFormatting.GRAY));
    } else if (element instanceof final ShortTag nbtShort) {
      return TextBridge.literal(String.valueOf(nbtShort.getAsShort())).append(TextBridge.literal("s").withStyle(ChatFormatting.GRAY));
    } else if (element instanceof final IntTag nbtInt) {
      return TextBridge.literal(String.valueOf(nbtInt.getAsInt()));
    } else if (element instanceof final LongTag nbtLong) {
      return TextBridge.literal(String.valueOf(nbtLong.getAsLong())).append(TextBridge.literal("l").withStyle(ChatFormatting.GRAY));
    } else if (element instanceof final FloatTag nbtFloat) {
      return TextBridge.literal(String.valueOf(nbtFloat.getAsFloat())).append(TextBridge.literal("f").withStyle(ChatFormatting.GRAY));
    } else if (element instanceof final DoubleTag nbtDouble) {
      return TextBridge.literal(String.valueOf(nbtDouble.getAsDouble())).append(TextBridge.literal("d").withStyle(ChatFormatting.GRAY));
    }
    return NbtUtils.toPrettyComponent(element);
  }

  /**
   * @see NbtUtils#toPrettyPrintedText
   */
  public static Component serialize(CompoundTag compound, int layer, String indent, int depth) {
    final Map<String, Tag> entries = ((NbtCompoundAccessor) compound).getEntries();
    switch (layer) {
      case 0 -> {
        ...
        for (Iterator<Map.Entry<String, Tag>> iterator = entries.entrySet().iterator();
          ...
          if (n >= 7) {
            ...
            final Map<String, Tag> remainsEntries =
