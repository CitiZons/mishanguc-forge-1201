 ...
 *
 * <pre>{@code
 * this.minecraft.setScreen(new TextPadEditScreen(entity))
 * }</pre>
 *
 * @param <T> 方块实体的类型。
 * @see net.minecraft.client.gui.screen.ingame.SignEditScreen
 * @see net.minecraft.client.network.LocalPlayer#openEditSignScreen
 * @see net.minecraft.server.network.ServerPlayer#openEditSignScreen
 */
@OnlyIn(Dist.CLIENT)
public abstract class AbstractSignBlockEditScreen<T extends BlockEntityWithText> extends Screen {
  ...
  public @NotNull TextFieldListWidget textFieldListWidget = new TextFieldListWidget(this, Minecraft.getInstance(), width, height, 25, height - 65, 18);
  ...
  public final SignPresetGridWidget signPresets = SignPresetGridWidget.createAllWidgets(this, Minecraft.getInstance(), height, 72, height - 68);
  ...
  public final FloatButtonWidget outlineColorButton = new FloatButtonWidget(0, 0, 70, 20, TextBridge.translatable("message.mishanguc.outline_color"), button -> {
    ...
    if (colorId == -1) {
      ...
      outlineColor = DyeColor.byId(colorId).getTextColor();
      outlineColorType = OutlineColorType.CUSTOM;
    }
    for (TextFieldListWidget.@NotNull Entry entry : textFieldListWidget.selectedEntries) {
    ...
    } else if (colorId == -3 && textFieldListWidget.getSelected() != null) {
      return MishangUtils.describeColor(textFieldListWidget.getSelected().textContext.outlineColor);
    } else if (colorId > 15 && textFieldListWidget.getSelected() != null) {
  ...
  public final Button customValueConfirmButton = Button.builder(CommonComponents.GUI_PROCEED, button -> {
      ...
      .setTooltipSupplier(value -> {
        if (!hidden && hasShiftDown() || textFieldListWidget.isSimplified()) {
          return Component.empty()
              .append(Component.translatable("message.mishanguc.simplify.height", textFieldListWidget.cuttingHeight).withStyle(style -> style.withColor(0xffd0d0d0)))
  ...
  protected void init() {
    ...
    if (!isAcceptingCustomValue && !isSelectingButtonToSetCustom) {
      Arrays.stream(toolboxTop).forEach(this::addWidget);
    }
    ...
    initSignPresets();
    if (!isAcceptingCustomValue && !isSelectingButtonToSetCustom) {
      this.addRenderableWidget(textFieldListWidget);
    } else {
      this.addRenderableOnly(textFieldListWidget);
    }

    if (!isAcceptingCustomValue && !isSelectingButtonToSetCustom) {
      Arrays.stream(toolboxTop).forEach(this::addRenderableOnly);
    }
  ...
  protected void initSignPresets() {
    ...
    signPresets.setRenderTopAndBottom(false);
  ...
  protected void updateContentVisibility() {
    ...
    if (!visible) {
      ...
      if (signPresets.getSelected() != null) {
        signPresets.getSelected().setFocused(false);
      }
  ...
  public void render(GuiGraphics context, int mouseX, int mouseY, float delta) {
    if (hidden) {
      ...
      MultiLineLabel.create(font, HIDDEN_TEXT_NOTE, width - 20).renderLeftAligned(context, 10, 10, 20, 0xffffffff);
      return;
    }
    ...
    if (isSelectingButtonToSetCustom) {
      ...
      MultiLineLabel.create(font, text, width - 20).renderLeftAligned(context, 10, 10, 20, 0xffdddddd);
  ...
  public boolean mouseClicked(double mouseX, double mouseY, int button) {
    ...
    for (GuiEventListener element : this.children()) {
      ...
      if (element instanceof SignPresetGridWidget || !textFieldListWidget.children().isEmpty()) {
        setFocused(textFieldListWidget);
      } else if (element instanceof AbstractSelectionList<?> || element instanceof EditBox) {
  ...
  public void removed() {
    ...
    ClientPlayNetworking.send(
        new ResourceLocation("mishanguc", "edit_sign_finish"),
        PacketByteBufs.create()
            .writeBlockPos(blockPos)
            .writeNbt(
                changed ? Util.make(new CompoundTag(), nbt -> nbt.put("texts", list)) : null));
  }
