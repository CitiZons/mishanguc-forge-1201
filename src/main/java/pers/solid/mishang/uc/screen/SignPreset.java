...
public record SignPreset(int order, String id, Component name, @Nullable Component description, List<TextContext> textContexts, int initialFocus) {
  ...
  public record Info(int order, Optional<Component> name, Optional<Component> description, List<TextContext> textContexts, int initialFocus) {
    public static final Codec<Info> CODEC = RecordCodecBuilder.<Info>create(i -> i.group(
        ...
        ExtraCodecs.COMPONENT.optionalFieldOf("name").forGetter(Info::name),
        ExtraCodecs.COMPONENT.optionalFieldOf("description").forGetter(Info::description),
     ...
     */
    public void save(String id, CommandSourceStack source, boolean force) {
      ...
      try {
        ...
        if (!filePath.getParent().equals(SignPresets.PATH)) {
          minecraft.execute(() -> source.sendFailure(Component.translatable("message.mishanguc.signPreset.save.invalid_name", id)));
          return;
        }
        if (!force && (Files.exists(filePath) || SignPresets.BUILTIN.containsKey(id))) {
          ...
          minecraft.execute(() -> source.sendFailure(Component.translatable("message.mishanguc.signPreset.save.fail.already_exist", Component.literal(newCommand).withStyle(style -> style
              .withUnderlined(true)
              .withColor(ChatFormatting.AQUA)
              .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.translatable("message.mishanguc.signPreset.save.fail.already_exist.click_to_type")))
        ...
        if (result.error().isPresent()) {
          minecraft.execute(() -> source.sendFailure(Component.translatable("message.mishanguc.signPreset.save.fail.encoding_failure")));
        ...
        minecraft.execute(() -> {
          ...
          source.sendSuccess(() -> Component.translatable("message.mishanguc.signPreset.save.success", Component.literal(filePath.getFileName().toString()).withStyle(style -> style
              .withUnderlined(true)
              ...
              .withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_FILE, filePath.toString())))), false);
        });
        ...
        minecraft.execute(() -> source.sendFailure(Component.translatable("message.mishanguc.signPreset.save.fail.unknown")));
     ...
     */
    public void delete(String id, CommandSourceStack source, boolean hideIfBuiltin) {
      final Minecraft minecraft = net.minecraft.client.Minecraft.getInstance();
      ...
      try {
        ...
        minecraft.execute(() -> {
          ...
          source.sendSuccess(() -> Component.translatable("message.mishanguc.signPreset.delete.success", Component.literal(filePath.getFileName().toString()).withStyle(ChatFormatting.YELLOW)), false);
        });
      } catch (InvalidPathException e) {
        minecraft.execute(() -> source.sendFailure(Component.translatable("message.mishanguc.signPreset.save.invalid_name", id)));
        SignPresets.LOGGER.error("Invalid path when deleting sign preset {}: {}", id, e.getMessage());
      } catch (IOException e) {
        minecraft.execute(() -> source.sendFailure(Component.translatable("message.mishanguc.signPreset.save.delete.unknown")));
